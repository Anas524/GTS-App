...
class MetalEntryController extends Controller
{
    ...
    {
        $files = collect($metalEntry->attachments ?? [])->map(function ($path) use ($metalEntry) {
            $name = basename($path);
            $ext  = strtolower(pathinfo($name, PATHINFO_EXTENSION));
            $type = in_array($ext, ['jpg','jpeg','png','webp','gif']) ? 'image' : 'pdf';
    
            $url = route('metals.attachments.preview', $metalEntry->id) . '?path=' . urlencode($path);
    ...
    public function attachmentsPreview(Request $request, MetalEntry $metalEntry)
    {
        $request->validate(['path' => ['required', 'string']]);
        $path = $request->path;
        ...
        // show inline in browser (image/pdf)
        return response()->file($abs, [
            'Content-Disposition' => 'inline; filename="' . basename($path) . '"'
        ]);
    }
    
    private function getItemsArray(MetalEntry $metalEntry): array
    {
        return is_array($metalEntry->items)
            ? $metalEntry->items
            : (json_decode($metalEntry->items ?? '[]', true) ?: []);
    }
    
    private function ensureItemIndex(array &$items, int $idx): void
    {
        if (!isset($items[$idx]) || !is_array($items[$idx])) $items[$idx] = [];
        if (!isset($items[$idx]['sell_attachments']) || !is_array($items[$idx]['sell_attachments'])) {
            $items[$idx]['sell_attachments'] = [];
        }
    }
    
    public function sellAttachmentsIndex(MetalEntry $metalEntry, int $idx)
    {
        $items = $this->getItemsArray($metalEntry);
        $list  = $items[$idx]['sell_attachments'] ?? [];
    
        $files = collect($list)->map(function ($path) use ($metalEntry, $idx) {
            $name = basename($path);
            $ext  = strtolower(pathinfo($name, PATHINFO_EXTENSION));
            $type = in_array($ext, ['jpg','jpeg','png','webp','gif']) ? 'image' : 'pdf';
    
            // IMPORTANT: use preview route (same like live entry attachments)
            $url = route('metals.sell.attachments.preview', [$metalEntry->id, $idx]) . '?path=' . urlencode($path);
    ...
    public function sellAttachmentsStore(Request $request, MetalEntry $metalEntry, int $idx)
    {
        ...
        $items = $this->getItemsArray($metalEntry);
        $this->ensureItemIndex($items, $idx);
    
        foreach ($request->file('attachments', []) as $file) {
            if (!$file) continue;
    
            $safeName = Str::random(6) . '_' . preg_replace('/\s+/', '_', $file->getClientOriginalName());
            $dir  = "metals/sell/{$metalEntry->id}/item_{$idx}";
    ...
    public function sellAttachmentsDestroy(Request $request, MetalEntry $metalEntry, int $idx)
    {
        ...
        // security: ensure requested path belongs to this item
        if (!in_array($path, $list, true)) abort(403);
    ...
    public function sellAttachmentsPreview(Request $request, MetalEntry $metalEntry, int $idx)
    {
        $request->validate(['path' => ['required', 'string']]);
        $path = $request->input('path');
