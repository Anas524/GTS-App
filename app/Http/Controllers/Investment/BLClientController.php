...
use App\Models\Investment\BLClient;
...
class BLClientController extends Controller
{
    ...
    {
        $clients = BLClient::orderBy('sr_no')->orderBy('sub_serial')->get();
    ...
    public function store(Request $request)
    {
        try {
            ...
            Log::error('BL STORE ERROR', ['error' => $e->getMessage()]);
            return response()->json(['success' => false, 'message' => $e->getMessage()], 500);
        }
    }

    public function saveBLClient(Request $request)
    {
        $latestSrNo = BLClient::max('sr_no') ?? 0;
    ...
    public function updateBLClient(Request $request, $id)
    {
        ...
        $client = BLClient::find($id);
        if (!$client) {
            return response()->json(['error' => 'BLClient not found'], 404);
        }
        ...
        return response()->json(['success' => true, 'message' => 'BLClient updated successfully.']);
    }
    ...
    public function deleteMultiple(Request $request)
    {
        try {
            ...
            $query = BLClient::where('date', $date);
            if (!is_null($supplier)) {
            ...
            if (strtolower($rawSubSerial) === 'all') {
                ...
                $deletedSrNo = BLClient::where('date', $date)
                    ->where('supplier_name', $supplier)
                    ->value('sr_no');

                $remaining = BLClient::where('sr_no', $deletedSrNo)
                    ->orderBy('sub_serial')
                    ->get();
    ...
    {
        $entry = BLClient::where('id', $id)->where('sub_serial', $sub_serial)->first();
    ...
    {
        $client = BLClient::where('id', $id)->where('sub_serial', $sub_serial)->first();
    ...
    {
        $clients = BLClient::orderBy('id')->get();
