...
use App\Models\Investment\RHClient;
...
class RHClientController extends Controller
{
    public function index()
    {
        $clients = RHClient::where('transaction_type', 'sheet_entry')
            ->orderBy('sr_no')
            ->orderBy('sub_serial')
            ->get();
    ...
    public function store(Request $request)
    {
        try {
            ...
            Log::error('RH STORE ERROR', ['error' => $e->getMessage()]);
            return response()->json(['success' => false, 'message' => $e->getMessage()], 500);
        }
    }

    public function saveRHClient(Request $request)
    {
    ...
    public function updateRHClient(Request $request, $id)
    {
        ...
        $shipping_rate = ($request->no_of_ctns ?? 0) * $weight * $shipping_rate_per_kg;
        ...
        return response()->json(['success' => true, 'message' => 'RHClient updated successfully.']);
    }
    ...
    public function deleteMultiple(Request $request)
    {
        try {
            ...
            $query = RHClient::where('date', $date);
            if (!is_null($supplier)) {
            ...
            if (strtolower($rawSubSerial) === 'all') {
                ...
                $deletedSrNo = RHClient::where('date', $date)
                    ->where('supplier_name', $supplier)
                    ->value('sr_no');

                $remaining = RHClient::where('sr_no', $deletedSrNo)
                    ->orderBy('sub_serial')
                    ->get();
    ...
    {
        $entry = RHClient::where('id', $id)->where('sub_serial', $sub_serial)->first();
    ...
    {
        $client = RHClient::where('id', $id)->where('sub_serial', $sub_serial)->first();
    ...
    {
        $clients = RHClient::orderBy('id')->get();
    ...
    public function storeLoan(Request $request)
    {
        ...
        DB::table('r_h_clients')->insert([
            'date' => $request->date,
            'description' => $request->description,
            'amount' => $request->amount,
            'transaction_type' => 'loan_entry',
            'created_at' => now(),
            'updated_at' => now(),
        ]);
    ...
    {
        $entries = DB::table('r_h_clients')
            ->where('transaction_type', 'loan_entry')
            ->orderBy('id', 'asc')
            ->get();
