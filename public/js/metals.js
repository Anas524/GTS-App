(function ($) {
    ...
    $(function () {
        ...
        function setSellAttachState($row) {
          const $detail = $row.closest('tr.pm-detail');
          const entryId = String($detail.data('detail-for') || '');
          const isSaved = /^\d+$/.test(entryId);
          const sold = isItemSold($row);
        
          const enable = sold && isSaved;
        
          const $up = $row.find('[data-action="sell-upload"]');
          const $vw = $row.find('[data-action="sell-view"]');
        
          $up.prop('disabled', !enable);
          $vw.prop('disabled', !enable);
        
          if (!isSaved) {
            $up.attr('title', 'Save this entry first to upload sell attachments.');
            $vw.attr('title', 'Save this entry first to view sell attachments.');
          } else if (!sold) {
            $up.attr('title', 'Add Sell Amount or Sell Date to enable Sell Upload.');
            $vw.attr('title', 'Add Sell Amount or Sell Date to enable Sell View.');
          } else {
            $up.attr('title', 'Upload sell attachments');
            $vw.attr('title', 'View sell attachments');
          }
        }
        ...
        // detect your 8 weights
        function weightKeyFromRow($tr){
          const wSel = String($tr.find('select[name$="[weight]"]').val() || '').trim();
          const wCustom = String($tr.find('.pm-weight-custom').val() || '').trim();
          const raw = (wSel === 'custom') ? wCustom : wSel;
        
          const norm = normalizeWeightToOptionValue(raw);
          const n = parseFloat(norm);
          if (!Number.isFinite(n)) return '';
        
          const near = (a,b,eps=0.0005) => Math.abs(a-b) <= eps;
        
          if (near(n, 1)) return '1g';
          if (near(n, 2.5)) return '2_5g';
          if (near(n, 5)) return '5g';
          if (near(n, 10)) return '10g';
          if (near(n, 20)) return '20g';
          if (near(n, 50)) return '50g';
          if (near(n, 100)) return '100g';
          if (near(n, 31.1035)) return '1oz';
        
          return '';
        }
        
        function autoCatalogImageSrc($tr){
          const metal = String($tr.find('select[name$="[metal_type]"]').val() || '').toLowerCase();
          const shape = String($tr.find('select[name$="[metal_shape]"]').val() || '').toLowerCase();
          if (metal !== 'gold' || shape !== 'bar') return null;
        
          const wKey = weightKeyFromRow($tr);  // returns 1g, 5g, 10g, 50g, 100g, 1oz...
          if (!wKey) return null;
        
          const bKey = brandKey($tr.find('input[name$="[brand_name]"]').val());
          if (!bKey) return null;
        
          // map whatever user types into your folder name
          const BRAND = {
                // PAMP
                'pamp': 'pamp',
                'pamp_suisse': 'pamp',
                ...
                // PERTH MINT
                'perth_mint': 'perthmint',
                'the_perth_mint': 'perthmint',
                'perthmint': 'perthmint',
          };
        
          const brandFolder = BRAND[bKey];
          if (!brandFolder) return null; // unknown brand => fallback to default goldbar.png
        
          // real structure:
          // Example filenames: 1g.png, 2_5g.png, 5g.png, 10g.png, 20g.png, 50g.png, 100g.png, 1oz.png
          return `/images/metals/catalog/gold/bar/${brandFolder}/${wKey}.png`;
        }
        ...
        function updateItemPreview($tr) {
            ...
            if (hasSell && isSaved && !unlocked) {
                ...
                // keep sell buttons enabled (closed but still upload/view allowed)
                // DO NOT disable sell-upload / sell-view here

                // keep image actions disabled in locked mode
                ...
                // enable image actions again when unlocked/not locked
            ...
            let src = previewSrc(metal, shape);               // default icon (always exists)
            let autoSrc = null;
            ...
            // AUTO ONLY if user did not upload a photo
            if (!hasCustom) {
              autoSrc = autoCatalogImageSrc($tr);
              if (autoSrc) src = autoSrc;
            }
            ...
            // mark state for CSS/zoom (no data stored)
            $wrap.toggleClass('is-custom', hasCustom);
            $wrap.toggleClass('is-auto', !hasCustom && !!autoSrc);
            
            // IMPORTANT: add safe onerror fallback (prevents broken icon)
            const fallbackSrc = previewSrc(metal, shape);
            $img.off('error.pmAuto').on('error.pmAuto', function(){
              $(this).attr('src', fallbackSrc);
              $wrap.removeClass('is-auto'); // optional
            });
        ...
        $tbody.on('input change', 'input[name$="[brand_name]"]', function () {
          const $tr = $(this).closest('tr[data-item-row]');
          updateItemPreview($tr);
        });
        ...
        function rowHasMetal($detail, metalKey) {
            if (!metalKey) return true;
            const items = getItemsForDetail($detail);
            return (items || []).some(it => String(it.metal_type || '').toLowerCase().trim() === metalKey);
        }
        ...
        function applySearchAndFilters(opts = {}) {
            ...
            $tbody.find('tr.pm-header[data-id]').each(function () {
                ...
                const passMetal = !metal || rowHasMetal($detail, metal);
        ...
        function isItemSold($row){
          const sellP = ($row.find('input[name$="[sell_price]"]').val() || '').trim();
          const sellD = ($row.find('input[name$="[sell_date]"]').val() || '').trim();
          return (num(sellP) > 0) || !!sellD;
        }
        
        function toggleSellButtons($detail){
          $detail.find('tr[data-item-row]').each(function () {
            setSellAttachState($(this));
          });
        }
